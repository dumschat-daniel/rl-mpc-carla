...
def get_slope_force(vehicle_info, vehicle_status):
    ...
    :rtype: float64
    """
    pitch = vehicle_status['orientation']['pitch']
...
def get_vehicle_max_steering_angle(vehicle_info):
    ...
    for wheel in vehicle_info.wheels:
        if wheel.max_steer_angle:
            if wheel.max_steer_angle and math.radians(wheel.max_steer_angle) < max_steering_angle:
                max_steering_angle = math.radians(wheel.max_steer_angle)
