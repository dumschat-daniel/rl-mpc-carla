import settings
from Dependencies import TRAINER_STATE_MESSAGE, AGENT_STATE_MESSAGE, STOP, STOP_MESSAGE, CarlaEnv, CARLA_SETTINGS_STATE, CARLA_SETTINGS_STATE_MESSAGE
import time
import colorama


'''
This was made for dqn and not updated for ddpg or td3, so console stats is not supposed for those currently.
'''
...
class ConsoleStats:
    ...
    def __init__(self, print_console, stop, duration, start_time, episode, epsilon, trainer_stats, agent_stats, episode_stats, carla_fps, weights_iteration, optimizer, carla_settings_threads, seconds_per_episode):
            ...
            if self.print_console.value:
                ...
                if settings.SHOW_CARLA_ENV_SETTINGS:
                    for process_no in range(settings.CARLA_HOSTS_NO):
                        ...
                        # Weather and NPCs
                        azimuth = f'{self.carla_settings_threads[process_no][1].weather._sun.azimuth:>7.2f}' if self.carla_settings_threads[process_no][1].weather else '    ---'
                        altitude = f'{self.carla_settings_threads[process_no][1].weather._sun.altitude:>7.2f}' if self.carla_settings_threads[process_no][1].weather else '    ---'
                        clouds = f'{self.carla_settings_threads[process_no][1].weather._storm.clouds:>6.2f}%' if self.carla_settings_threads[process_no][1].weather else '    ---'
                        wind = f'{self.carla_settings_threads[process_no][1].weather._storm.wind:>6.2f}%' if self.carla_settings_threads[process_no][1].weather else '    ---'
                        rain = f'{self.carla_settings_threads[process_no][1].weather._storm.rain:>6.2f}%' if self.carla_settings_threads[process_no][1].weather else '    ---'
                ...
                time.sleep(settings.PRINT_CONSOLE_EVERY)
            else:
                time.sleep(10.0)
